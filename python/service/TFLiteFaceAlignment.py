...
if __name__ == '__main__':
    ...
    faceDetecion = UltraLightFaceDetecion(
        "models/RFB-320.tflite",
        conf_threshold=0.88)
    coordinateAlignment = CoordinateAlignmentModel(
        "models/coor_2d106.tflite")
    ...
    while True:
        ...
        if not ret:
            print('정상적으로 frame을 얻을 수 없음.')
            break

        boxes, scores = faceDetecion.inference(frame)

        print("랜드마크 획득")
        start_time = time.perf_counter()


        for pred in coordinateAlignment.get_landmarks(frame, boxes):
            print(pred)
